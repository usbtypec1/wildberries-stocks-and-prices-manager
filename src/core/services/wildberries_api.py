...
class WildberriesAPIService:
    ...
    def __init__(self, api_client: httpx.Client, console: Console):
        self.__api_client = api_client
        self.__console = console

    def update_stocks(self, warehouse_id: int, stocks: Iterable[
        core.models.wildberries_api.StocksBySku]):
        url = f'/api/v3/stocks/{warehouse_id}'
        request_data = {
            'stocks': [{'sku': stock.sku, 'amount': stock.amount} for stock in
                       stocks]
        }
    ...
    tuple[
        ...
        while True:
            ...
            if response.status_code == 429:
                print('Тротлинг сервера Wildberries API')
                time.sleep(5)
                continue
            response_data = response.json()
            return parse_obj_as(
                tuple[core.models.wildberries_api.StocksBySku, ...],
                response_data['stocks'])

    def iter_nomenclatures(self) -> Generator[
        list[models.Nomenclature], None, None]:
        ...
        while True:
            ...
            if pagination_data is not None:
                body['sort']['cursor'] = body['sort'][
                                             'cursor'] | pagination_data
            for _ in range(5):
                with contextlib.suppress(Exception):
                    response = self.__api_client.post(
                        '/content/v1/cards/cursor/list', json=body)
                    time.sleep(1)
            ...
            self.__console.log(f'Загружена информация о {total} товарах')
    ...
    def get_prices(self, quantity_status: models.QuantityStatus) -> tuple[
        models.NomenclaturePrice, ...]:
